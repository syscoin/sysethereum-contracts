...
contract('SyscoinSuperblocks', (accounts) => {
  ...
  describe('Utils', () => {
    ...
    it('Superblock id', async () => {
      ...
      const retargetPeriod = "0x000000000000000000000000000000000000000000000000000000005ada05b9";
      ...
      const id = await superblocks.calcSuperblockHash(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, parentId, 1);
  ...
  describe('Verify status transitions', () => {
    ...
    it('Initialized', async () => {
      const result = await superblocks.initialize(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, parentHash, 0,{ from: claimManager });
      assert.equal(result.logs[0].event, 'NewSuperblock', 'New superblock proposed');
      id0 = result.logs[0].args.superblockHash;
    });
    it('Propose', async () => {

      const result = await superblocks.propose(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, id0, claimManager, utils.ZERO_ADDRESS,{ from: claimManager });
      assert.equal(result.logs[0].event, 'NewSuperblock', 'New superblock proposed');
    ...
    it('Bad parent', async () => {
      const result = await superblocks.propose(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, "0x0", claimManager, utils.ZERO_ADDRESS,{ from: claimManager });
    ...
    it('Propose bits', async () => {
      const result = await superblocks.propose(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, id1, claimManager, utils.ZERO_ADDRESS,{ from: claimManager });
      assert.equal(result.logs[0].event, 'NewSuperblock', 'New superblock proposed');
    ...
    it('Propose tris', async () => {
      const result = await superblocks.propose(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, id2, claimManager, utils.ZERO_ADDRESS,{ from: claimManager });
      assert.equal(result.logs[0].event, 'NewSuperblock', 'New superblock proposed');
  ...
  describe('Only ClaimManager can modify', () => {
    ...
    const retargetPeriod = timestamp - 1;
    const lastBits = 1;
    const lastHash = '0x00';
    ...
    it('Initialized', async () => {
      const result = await superblocks.initialize(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, parentHash,0);
      assert.equal(result.logs[0].event, 'NewSuperblock', 'New superblock proposed');
      id0 = result.logs[0].args.superblockHash;
    });
    it('Propose', async () => {
      let result = await superblocks.propose(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, id0, claimManager,utils.ZERO_ADDRESS);
      assert.equal(result.logs[0].event, 'ErrorSuperblock', 'Only claimManager can propose');
      result = await superblocks.propose(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, id0, claimManager, utils.ZERO_ADDRESS,{ from: claimManager });
    ...
    it('Challenge', async () => {
      let result = await superblocks.propose(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, id1, claimManager, utils.ZERO_ADDRESS,{ from: claimManager });
      assert.equal(result.logs[0].event, 'NewSuperblock', 'ClaimManager can propose');
      id2 = result.logs[0].args.superblockHash;
    ...
    it('Invalidate', async () => {
      let result = await superblocks.propose(merkleRoot, accumulatedWork, timestamp, retargetPeriod, lastHash, lastBits, id2, claimManager, utils.ZERO_ADDRESS,{ from: claimManager });
      assert.equal(result.logs[0].event, 'NewSuperblock', 'New superblock proposed');
      id3 = result.logs[0].args.superblockHash;
  ...
  describe('Test locator', () => {
    ...
    it('Verify locator', async () => {
      ...
      for(let work = 1; work < 30; ++work) {
        result = await superblocks.propose(merkleRoot, work, 0, 0, lastHash, lastBits, parentId, claimManager, utils.ZERO_ADDRESS,{ from: claimManager });
        assert.equal(result.logs[0].event, 'NewSuperblock', 'ClaimManager can propose');
