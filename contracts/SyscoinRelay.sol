...
import "./interfaces/ISyscoinRelay.sol";
import "./interfaces/ISyscoinTransactionProcessor.sol";
import "./lib/SyscoinMessageLibrary.sol";
...
contract SyscoinRelay is ISyscoinRelay, SyscoinMessageLibrary {
    ...
    ISyscoinTransactionProcessor public syscoinVaultManager;
    ...
    function init(address _syscoinVaultManager) external {
        ...
        syscoinVaultManager = ISyscoinTransactionProcessor(
            _syscoinVaultManager
        );
        initialized = true;
    }
    ...
    function parseFirstAssetCommitmentInTx(
        bytes memory txBytes,
        uint pos,
        int opIndex
    ) internal pure returns (uint, uint64, uint) {
        uint numAssets;
        uint assetGuid;
        uint64 assetGuid64;
        uint bytesToRead;
        ...
        for (uint assetIndex = 0; assetIndex < numAssets; assetIndex++) {
            if (assetIndex == 0) {
                ...
                if (opIndex >= 0) {
                    require(
                        output_value > 0,
                        "#SyscoinRelay parseFirstAssetCommitmentInTx(): output index not found"
                    );
        ...
        )
    {
        ...
        (output_value, assetGuid, pos) = parseFirstAssetCommitmentInTx(
            txBytes,
            pos,
            int(opIndex)
        );
        ...
        require(
            numBytesInAddress == 0x14,
            "#SyscoinRelay scanBurnTx(): Invalid destinationAddress"
        );
    ...
    function getOpReturnPos(
        ...
        if (n_inputs == 0x00) {
            ...
            require(
                n_inputs != 0x00,
                "#SyscoinRelay getOpReturnPos(): Unexpected dummy/flag"
            );
            (n_inputs, pos) = parseCompactSize(txBytes, pos);
        }
        require(
            n_inputs < 100,
            "#SyscoinRelay getOpReturnPos(): Incorrect size of n_inputs"
        );
        ...
        require(
            n_outputs < 10,
            "#SyscoinRelay getOpReturnPos(): Incorrect size of n_outputs"
        );
    ...
    function verifyTx(
        ...
        if (
            helperVerifyHash(txHash, _txIndex, _siblings, _blockHeaderBytes) ==
            1
        ) {
            return txHash;
        } else {
    ...
    function verifySPVProofs(
        ...
        (bool success, bytes memory result) = SYSBLOCKHASH_PRECOMPILE_ADDRESS
            .staticcall{gas: SYSBLOCKHASH_PRECOMPILE_COST}(input);
        require(success, "SYSBLOCKHASH precompile call failed.");
        require(
            result.length > 0,
            "SYSBLOCKHASH precompile returned empty result."
        );
    ...
    function relayTx(
        ...
        // pass assetGuid
        syscoinVaultManager.processTransaction(
            txHash,
            value,
            destinationAddress,
            assetGuid
        ...
        )
    {
        ...
        (output_value, destinationAddress, assetGuid) = scanBurnTx(
            txBytes,
            opIndex,
            pos
        );
    ...
    function dblSha(bytes memory _dataBytes) internal pure returns (uint) {
        return
            uint(
                sha256(abi.encodePacked(sha256(abi.encodePacked(_dataBytes))))
            );
    }
